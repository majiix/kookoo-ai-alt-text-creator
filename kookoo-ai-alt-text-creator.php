...
class Aialtg_Image_Descriptor {
	...
	public function handle_fix_json_errors() {
		...
		if ( $results ) {
			foreach ( $results as $row ) {
				// We don't delete the bad text, we just reset the 'processed' flag.
				// The Cron will overwrite the text on next run.
				delete_post_meta( $row->ID, '_aialtg_processed' );
				// Clean up error log too if it exists.
				delete_post_meta( $row->ID, '_aialtg_error_log' );
				$count++;
			}
	...
	public function enqueue_admin_assets( $hook ) {
		...
		wp_enqueue_style( 'aialtg-admin-css', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), '1.7.0' );
		wp_enqueue_script( 'aialtg-admin-js', plugins_url( 'assets/js/admin.js', __FILE__ ), array( 'jquery' ), '1.7.0', true );
